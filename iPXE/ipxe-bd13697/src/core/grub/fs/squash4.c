	       ...
	       char *outbuf, grub_size_t len, struct grub_squash_data *data)
{
  ...
  while (len)
	...
	{
	  grub_error (GRUB_ERR_BAD_COMPRESSED_DATA, "invalid xz chunk xzret=%d insize=%u len=%u", 
        xzret, (grub_uint32_t)insize, (grub_uint32_t)len);
	  return -1;
	}
...
squash_mount (grub_disk_t disk)
{
  ...
  if (grub_errno == GRUB_ERR_OUT_OF_RANGE)
    grub_error (GRUB_ERR_BAD_FS, "not a squash4 1");
  if (err)
    return NULL;
  if (sb.magic != grub_cpu_to_le32_compile_time (SQUASH_MAGIC)
    ...
    {
      grub_error (GRUB_ERR_BAD_FS, "not squash4 2");
      return NULL;
    }
  ...
  if (grub_errno == GRUB_ERR_OUT_OF_RANGE)
    grub_error (GRUB_ERR_BAD_FS, "not a squash4 3");
  if (err)
    return NULL;
