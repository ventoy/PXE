...
static const struct grub_arg_option options[] = {
  ...
  {"length", 'n', 0, N_("length to checksum."), 0, 0},
  {0, 0, 0, 0, 0, 0}
};
#endif
...
static grub_err_t
hash_file (grub_file_t file, grub_uint64_t checklen, const gcry_md_spec_t *hash, void *result)
{
  int progress = 0;
  grub_uint64_t ro = 0;
  grub_uint64_t div = 0;
...
#define BUF_SIZE 64 * 1024
  ...
  while (total < dstlen)
	...
	break;
      ...
      {
          total += r;
          div = grub_divmod64(total * 100, dstlen, &ro);
	    ...
	    const char *prefix, int keep, int uncompress)
{
  ...
  while (grub_free (buf), (buf = grub_file_getline (hashlist)))
      ...
      if (!file)
	{
      ...
      err = hash_file (file, 0, hash, actual);
		  ...
		  int argc, char **args)
{
  ...
  if (state[5].set)
    checklen = grub_strtoul(state[5].arg, NULL, 10);
  ...
  for (i = 0; i < (unsigned) argc; i++)
      ...
      if (!file)
	{
      ...
      err = hash_file (file, checklen, hash, result);
      grub_file_close (file);
      if (err)
	{
	  if (!keep)
