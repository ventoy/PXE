...
static struct option_descriptor echo_opts[] = {
	OPTION_DESC ( "n", 'n', no_argument, struct echo_options, no_newline, parse_flag ),
	OPTION_DESC ( "s", 's', no_argument, struct echo_options, success, parse_flag ),
	OPTION_DESC ( "w", 'w', no_argument, struct echo_options, warning, parse_flag ),
	OPTION_DESC ( "e", 'e', no_argument, struct echo_options, error, parse_flag ),
};
...
static int echo_exec ( int argc, char **argv ) {
    ...
    if (opts.success)
    {
        printf ("\033[32m" "%s%s" "\033[0m", text, ( opts.no_newline ? "" : "\n" ) );   
    }
    else if (opts.warning)
    {
        printf ("\033[33m" "%s%s" "\033[0m", text, ( opts.no_newline ? "" : "\n" ) );   
    }
    else if (opts.error)
    {
        printf ("\033[31m" "%s%s" "\033[0m", text, ( opts.no_newline ? "" : "\n" ) );   
    }
    else
    {
