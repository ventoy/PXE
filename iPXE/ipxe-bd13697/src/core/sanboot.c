...
struct san_device * sandev_first(void) {
    struct san_device *sandev;
    list_for_each_entry ( sandev, &san_devices, list ) {
		return sandev;
	}
...
int sandev_read ( struct san_device *sandev, uint64_t lba,
    ...
    int i;
	int rc;
    uint64_t seek, len, rangeStart, rangeEnd, rangeLen, overLen;
    ...
    /* do override */
    {
        seek = lba * sandev_blksize(sandev);
        len = count * sandev_blksize(sandev);

        for (i = 0; i < g_aiSandiskOverCnt; i++)
        {
            rangeStart = g_aui64SandiskOverLoc[i];
            rangeLen = g_uiSandiskOverLen;
            rangeEnd = rangeStart + rangeLen;
        
            if (seek + len <= rangeStart || seek >= rangeEnd)
            {
                /* no override */
            }
            else if (seek <= rangeStart)
            {
                if (seek + len <= rangeEnd)
                {
                    overLen = seek + len - rangeStart;
                }
                else
                {
                    overLen = rangeLen;
                ...
                memcpy((char *)buffer + rangeStart - seek, g_pcSandiskOverBuf, overLen);
            }
            else
            {
                if (seek + len <= rangeEnd)
                ...
                {
                    overLen = rangeEnd - seek;
                }
            
                memcpy((char *)buffer, g_pcSandiskOverBuf + seek - rangeStart, overLen);
            }
        }
    }
