...
static void sync_console ( struct edit_string *string ) {
	...
	while ( cursor < mod_end ) {
		putchar ( ( cursor >= len ) ? ' ' : (g_password_flag ? '*' : string->buf[cursor]) );
		cursor++;
	}
 ...
 */
int readline_history_real ( const char *prompt, const char *prefill,
		       struct readline_history *history, unsigned long timeout,
		       char **line, int EscShellExit ) {
...
int readline_history ( const char *prompt, const char *prefill,
		       struct readline_history *history, unsigned long timeout,
		       char **line ) {
    return readline_history_real(prompt, prefill, history, timeout, line, 0);
}
