...
static uint8_t dhcp_request_options_data[] = {
	...
	DHCP_USER_CLASS_ID, DHCP_STRING ( 'i', 'V', 'e', 'n', 't', 'o', 'y' ),
...
/* longpanda: the boot file name must in 01-xx-xx-xx format */
if (DHCPOFFER == msgtype) 
{
    if (!vtoy_is_bootfilename_valid(dhcppkt->dhcphdr->file, inet_ntoa(server_id)))
    {
        return;
    }
}
...
int dhcp_create_packet ( struct dhcp_packet *dhcppkt,
    ...
    {
        extlen = toy_dhcp_discover_extlen();
    }

	/* Sanity check */
	if ( max_len < ( sizeof ( *dhcphdr ) + options_len + extlen) )
		return -ENOSPC;
    ...
    if (extlen > 0)
    {
        toy_dhcp_discover_fill_ext(dhcphdr->options + options_len - 1);
        dhcphdr->options[options_len + extlen - 1] = 255;
    }
