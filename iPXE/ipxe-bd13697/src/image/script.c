...
static int process_script ( struct image *image,
    ...
    /* longpanda: support contine line */
    tmp = (char *)image->data;
    for (frag_len = 0; frag_len < image->len; frag_len++)
    {
        if (tmp[frag_len] == '\n')
        {
            if (frag_len > 0 && tmp[frag_len - 1] == '\\')
            {
                tmp[frag_len - 1] = ' ';
                tmp[frag_len] = ' ';
            }
            else if (frag_len > 1 && tmp[frag_len - 1] == '\r' && tmp[frag_len - 2] == '\\')
            {
                tmp[frag_len - 2] = ' ';
                tmp[frag_len - 1] = ' ';
                tmp[frag_len] = ' ';
