...
#undef DBGC
#undef DBGC2
#define DBGC(a, fmt, ...)  printf(fmt, ##__VA_ARGS__)
#define DBGC2(a, fmt, ...)  (void)a; printf(fmt, ##__VA_ARGS__)
#endif
...
static __asmcall __used void int13 ( struct i386_all_regs *ix86 ) {
    ...
    /* VTOY: We simulate a cdrom, so no need to sync hd drive number */
...
static int int13_hook ( unsigned int drive, struct uri **uris,
    ...
    /* VTOY: hook will copy num_drives to dl when int13 08 was called, so must initialize it's value */
	get_real(num_drives, BDA_SEG, BDA_NUM_DRIVES);
    ...
    /* VTOY: directly use drive number */
    natural_drive = drive;
