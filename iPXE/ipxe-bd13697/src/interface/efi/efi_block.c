...
efi_block_io_read ( EFI_BLOCK_IO_PROTOCOL *block_io, UINT32 media __unused,
	...
	//dbg_printf ( "EFIBLK %#02x read LBA %#08llx to %p+%#08zx ...\n",
   ...
   //dbg_printf ( "EFIBLK %#02x read LBA %#08llx to %p+%#08zx OK\n",
   //	sandev->drive, lba, data, ( ( size_t ) len ) );
...
static int efi_block_hook ( unsigned int drive, struct uri **uris,
    ...
    efirc = vtoy_connect_driver(block->handle, L"Disk I/O Driver");
    vdebug("Connect disk IO driver %s\n", efi_status(efirc));

    efirc = vtoy_connect_driver(block->handle, L"Partition Driver");
    vdebug("Connect partition driver %s\n", efi_status(efirc));
    if (efirc)
    {
...
static int efi_block_boot ( unsigned int drive, const char *filename ) {
	...
	for ( i = 0 ; i < count ; i++ ) {
        ...
        efirc = bs->HandleProtocol(handles[i], &efi_simple_file_system_protocol_guid, (VOID **)&pFile);
        if (efirc == 0)
        {
